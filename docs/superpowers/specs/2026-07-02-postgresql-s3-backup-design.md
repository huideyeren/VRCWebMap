...
リポジトリのルートディレクトリで、次のコマンドを実行する。

```sh
docker compose --env-file .env.backup --profile backup run --rm db-backup backup
```

このコマンドはバックアップの作成とアップロードに成功した後、`BACKUP_RETENTION_COUNT` を超えた完了済み世代を削除する。cronとWindowsタスクスケジューラーでも同じコマンドを使う。

### バックアップ一覧

完了済み世代と、リストアで指定できるobject keyを表示する。
...
どちらのリストア用ラッパーも内部で次のComposeコマンドを実行する。利用者がこの内部コマンドを直接実行するとバックエンドの停止と復旧制御を迂回するため、通常運用では使用しない。

```sh
docker compose --env-file .env.backup --profile backup run --rm db-backup restore "<resolved-object-key>"
```
