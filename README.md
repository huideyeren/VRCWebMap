...
The `backup` Compose profile provides an on-demand PostgreSQL 18 client with
AWS CLI v2. It does not start during a normal `docker compose up`.

Copy `.env.backup.example` to the ignored `.env.backup` file, restrict its file
permissions, and set the PostgreSQL and S3 credentials. For Wasabi, set
`S3_ENDPOINT_URL` to the regional endpoint such as
`https://s3.ap-northeast-1.wasabisys.com` and normally use
...
Each complete generation consists of a PostgreSQL custom `.dump`, `.sha256`,
and `.json` manifest. The manifest is uploaded last, so objects without a valid
manifest are ignored. Four complete generations are retained by default.

Restore an exact generation or the latest validated generation with:
...
PowerShell 7 uses the equivalent wrapper:

```powershell
.\ops\db-backup\restore.ps1 latest
.\ops\db-backup\restore.ps1 "backups/vrcwebmap-20260702T030000Z.dump"
```

The wrapper resolves and describes the manifest before stopping the backend,
then requires the exact confirmation text `vrcwebmap`. A failed restore leaves
the backend stopped. Restart it manually after investigating:

```bash
docker compose up --detach backend
```

The S3 identity needs `s3:ListBucket` on the configured bucket and
`s3:GetObject`, `s3:PutObject`, and `s3:DeleteObject` on the configured prefix.
Do not grant access to unrelated buckets. Linux can use
`ops/db-backup/cron.example` for a Sunday 03:00 run; Windows Task Scheduler can
invoke `ops/db-backup/windows-task.example.ps1`. Replace the repository paths
with absolute production paths.
...
KML export includes a direct link back to each source Spot. Set the externally
reachable base URL (including any reverse-proxy path prefix) before enabling
the feature in production:

```text
App__PublicBaseUrl=https://maps.example.com
```

The exported KML can be imported into Google My Maps or uMap. It retains the
Spot description, source, registrant display name, and the direct link. It is
not an OpenStreetMap API posting format and does not submit data to OSM.
...
Compose の `backup` profile は、PostgreSQL 18 client と AWS CLI v2 を使う
オンデマンドのバックアップ環境です。通常の `docker compose up` では起動しません。

`.env.backup.example` を、Git管理対象外の `.env.backup` へコピーし、ファイル権限を
制限してPostgreSQLとS3の資格情報を設定します。Wasabiでは地域endpoint
（例: `https://s3.ap-northeast-1.wasabisys.com`）を `S3_ENDPOINT_URL` に指定し、
通常は `S3_ADDRESSING_STYLE=path` を使います。
...
完全な1世代は、PostgreSQL custom形式の `.dump`、`.sha256`、`.json` manifestで
構成します。manifestを最後にuploadするため、有効なmanifestがないobjectは一覧・復元対象に
なりません。既定では完全な4世代を保持します。

検証済みの最新世代または明示した世代を復元します。
...
wrapperはbackendを停止する前にmanifestを解決・表示し、完全一致する確認文字列
`vrcwebmap` を要求します。復元に失敗した場合、backendは停止したままです。原因を調査してから
手動で再起動してください。
...
S3の資格情報には、対象bucketの `s3:ListBucket` と、対象prefixの
`s3:GetObject`、`s3:PutObject`、`s3:DeleteObject` だけを付与します。
週次実行例はLinux向け `ops/db-backup/cron.example` とWindows Task Scheduler向け
`ops/db-backup/windows-task.example.ps1` にあります。repository pathは本番環境の
絶対pathへ置き換えてください。
...
- 右上のズーム操作とレイヤー切替から、細かい縮尺と標準地図・国土地理院航空写真を選べます。
- 地図画面のKML移行・出力パネルでは、選択したSpotをGoogle My Maps/uMap向けKMLとして出力できます。ログイン済みでVRChat表示名を登録した利用者は、KML/KMZをプレビューしてSpotとして取り込めます。既存Spotから50m以内の候補は、明示的に選択するまで取り込みません。
