...
`wwwroot/` contains a temporary React + Leaflet frontend served by ASP.NET Core
...
- The temporary UI currently uses CDN-delivered React and Leaflet. A Node.js
  frontend build and OpenAPI-generated client can replace this later.
- When the Node.js frontend is introduced, use PNPM for dependency management
  and commit `pnpm-lock.yaml`. Do not use `npm install` or `npm ci` for this
  project.
...
- While this temporary CDN-based UI remains, ASP.NET Core sends a Content
  Security Policy that restricts scripts, styles, connections, and map tiles to
  the current origin and the explicitly used providers. Leaflet CDN assets use
  SRI, and React URLs are version-pinned.
...
`wwwroot/` には、ASP.NET Core の静的ファイル配信で表示する仮の React + Leaflet
フロントエンドを置いています。

- 地図を右クリックすると、その位置の Spot 登録フォームを表示します。
- Spot marker をクリックすると、右ペインに Spot と関連データを表示します。
- 現時点の仮 UI は CDN 配信の React と Leaflet を使います。後で Node.js frontend build と
  OpenAPI 生成 client に置き換える想定です。
- Node.js frontend を導入する場合、依存管理には PNPM を使い、`pnpm-lock.yaml` をコミットします。
  このプロジェクトでは `npm install` / `npm ci` は使いません。
...
- この CDN ベースの仮 UI を使う間は、ASP.NET Core が Content Security Policy を返し、
  script、style、接続先、地図 tile provider を現在の origin と明示的に使う provider に制限します。
  Leaflet の CDN asset には SRI を付け、React の URL はバージョン固定します。
